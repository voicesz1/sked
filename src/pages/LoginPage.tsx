...
export const LoginPage = ({ navigate, store }: any) => {
  const handleLogin = (e: any) => {
    ...
    store.setCurrentUser({ name: 'Admin User', email: 'admin@barber.com' });
    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-[var(--background)]">
      <Card className="w-full max-w-md p-8">
        ...
        <form onSubmit={handleLogin} className="space-y-4">
          <Input label="Email" type="email" placeholder="admin@exemplo.com" defaultValue="admin@barber.com" />
          <Input label="Senha" type="password" placeholder="••••••••" defaultValue="123456" />
  ...
  <div className="min-h-screen flex items-center justify-center p-4 bg-[var(--background)]">
    <Card className="w-full max-w-md p-8">
      ...
      <div className="space-y-4">
        <Input label="Nome da Empresa" placeholder="Ex: Viking Cuts" />
        <Input label="Email" type="email" />
        <Input label="Senha" type="password" />
        <Button onClick={() => navigate('/login')} className="w-full h-12">Registrar</Button>
      </div>
